...
class WangYiYunSpider:
    ...
    def get_playlist(self, html):
        ...
        try:
            next_url = html.xpath('//a[@class="zbtn znxt"]/@href')[0] if len(
                html.xpath('//a[@class="zbtn znxt"]/@href')) > 0 else None
            print(next_url)
        except:
            return None
        else:
    ...
    def get_playlist_info(self):
        ...
        for url in self.playlist_urls:
            ...
            for li in songs_li:
                song_info = {
                                'song_name': li.xpath('.//text()') if len(li.xpath('.//text()')) > 0 else None,
                                'song_link': li.xpath('./a/@href') if len(li.xpath('./a/@href')) > 0 else None
            ...
            info_dict = {
                ...
                'title': html.xpath('//title/text()')[0] if len(html.xpath('//title/text()')) > 0 else None,
                'url': url,
                'author': html.xpath('//div[@class="user f-cb"]/a[@class="s-fc7"]/text()')[0] if len(
                    html.xpath('//div[@class="user f-cb"]/a[@class="s-fc7"]/text()')) > 0 else None,
                'create_time': html.xpath('//div[@class="user f-cb"]/span[@class="u-icn u-icn-84 "]/text()')[0] if len(
                    html.xpath('//div[@class="user f-cb"]/span[@class="u-icn u-icn-84 "]/text()')) > 0 else None,
                'tags': html.xpath('//div[@clas="tags f-cb"]/b//text()')[0] if len(
                    html.xpath('//div[@clas="tags f-cb"]/b//text()')) > 0 else None,
                'description': html.xpath('//p[@id="album-desc-dot"]/text()')[0] if len(
                    html.xpath('//p[@id="album-desc-dot"]/text()')) > 0 else None,
                'transmit': html.xpath('//a[@class="u-btni u-btni-share "]/i/text()')[0] if len(
                    html.xpath('//a[@class="u-btni u-btni-share "]/i/text()')) > 0 else None,
                'store': html.xpath('//a[@class="u-btni u-btni-fav "]/i/text()')[0] if len(
                    html.xpath('//a[@class="u-btni u-btni-fav "]/i/text()')) > 0 else None,
                'comments': html.xpath('//span[@id="cnt_comment_count"]/text()')[0] if len(
                    html.xpath('//span[@id="cnt_comment_count"]/text()')) > 0 else None,
                'played_times': html.xpath('//strong[@id="play-count"]/text()')[0] if len(
                    html.xpath('//strong[@id="play-count"]/text()')) > 0 else None,
                'songs': songs
            }
    ...
    def run(self):
        ...
        for url in self.class_url_list:
            ...
            html = etree.HTML(html_str)
            # 小类名 作为文件名
            self.classname = html.xpath('//span[@class="f-ff2 d-flag"]/text()')[0] if len(
                html.xpath('//span[@class="f-ff2 d-flag"]/text()')[0]) > 0 else None
